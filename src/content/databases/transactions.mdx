...
Pick an anomaly and an isolation level and watch two transactions interleave —
the critical read lights green when the level prevents it, rose when it slips
through. Run **Non-repeatable** at Read Committed (it slips through), then raise it
to Repeatable Read and watch it vanish:

<Visualizer algo="isolation" />
...
Concurrency is where databases get hard. Whiteboard each before revealing the
checklist.
...
<DesignDrills
  ...
  drills={[
    {
      level: "Warm-up",
      prompt: "Two users withdraw from the same account at the same time. Show the lost-update bug, then the one-line fix.",
      covers: [
        "Both read balance, both compute, both write → one update is silently lost.",
        "Fix in one statement: UPDATE ... SET balance = balance - x WHERE id = ? AND balance >= x.",
        "Or SELECT ... FOR UPDATE to serialize, or an optimistic version check + retry.",
        "The race is a read-modify-write — close it at the point of mutation."
      ]
    },
    {
      level: "Core",
      prompt: "Name the lowest isolation level that prevents each: dirty read, non-repeatable read, phantom.",
      covers: [
        "Dirty read → Read Committed.",
        "Non-repeatable read → Repeatable Read.",
        "Phantom → Serializable (SQL standard; PostgreSQL's RR also stops phantoms via MVCC).",
        "Higher level = safer but less concurrency / more retries."
      ]
    },
    {
      level: "Core",
      prompt: "Pessimistic (SELECT FOR UPDATE) vs optimistic (version column) locking — when do you choose each?",
      covers: [
        "Pessimistic: high contention, expensive-to-retry work, long critical sections.",
        "Optimistic: low contention, cheap retries, read-heavy workloads.",
        "Optimistic needs retry-on-conflict (WHERE version = ? → rowcount 0 means retry).",
        "Pessimistic risks deadlocks and blocking; keep the transaction short."
      ]
    },
    {
      level: "Stretch",
      prompt: "Two transactions deadlock. What does the database do, and how do you prevent it?",
      covers: [
        "The DB detects the lock cycle, aborts a victim with a deadlock error → the app retries.",
        "Prevent with consistent lock ordering (always lock by ascending id).",
        "Keep transactions short; acquire all needed locks upfront.",
        "Retry with jittered backoff — deadlocks are expected, not exceptional."
      ]
    },
    {
      level: "Stretch",
      prompt: "A multi-step operation spans two services, so you can't wrap it in one DB transaction. Now what?",
      covers: [
        "A saga: a sequence of local transactions with compensating actions to undo.",
        "Idempotency keys so a retried step doesn't double-apply.",
        "A transactional outbox to publish events reliably with each local commit.",
        "Accept eventual consistency — distributed ACID across services isn't free."
      ]
    }
  ]}
/>
...
<ThinkThrough
  ...
  difficulty="Database Concurrency"
  problem="A credit system runs transactions concurrently. Tracing raw read/write timestamps, derive how the database identifies a Dirty Read, a Non-Repeatable Read, and a Phantom Read."
  stages={[
    {
      label: "Dirty Read (Read Uncommitted)",
      prompt: "Explain the timeline where Transaction A updates balance to $600, Transaction B reads the balance as $600, and Transaction A then aborts (Rollback). What is the anomaly?",
      reveal: "This is a Dirty Read. Transaction B read uncommitted data written by Transaction A. Once Transaction A rolled back, that write vanished, leaving B with dirty data that never existed. This is prevented under Read Committed."
    },
    {
      label: "Non-Repeatable Read (Read Committed)",
      prompt: "Under the Read Committed level, Transaction B queries the account balance and sees $500. Transaction A updates the balance to $400 and COMMITS. Transaction B runs the exact same query in the same transaction and gets $400. Why does this occur?",
      reveal: "This is a Non-Repeatable Read. Under Read Committed, each individual SELECT statement generates a new snapshot of committed data. Since Transaction A committed its update before B's second SELECT, B sees the new value. To prevent this, the transaction must be elevated to Repeatable Read (which pins a single snapshot at the start)."
    },
    {
      label: "Phantom Read (Repeatable Read vs Serializable)",
      prompt: "How does a Phantom Read differ from a Non-Repeatable Read when querying database tables?",
      reveal: "A Non-Repeatable Read concerns updates/deletions to *existing rows* that the transaction has already read. A Phantom Read occurs when a transaction queries a *set of rows* matching a condition (e.g. active users), and a concurrent transaction commits an INSERT/DELETE of a *new row* matching that condition, causing the set's size to change when re-queried."
    }
  ]}
/>
...
<Quiz questions={[
  {
    question: "Under the 'Atomicity' guarantee of ACID, what happens if a database crashes halfway through executing a transaction?",
    options: [
      "The database keeps the successfully executed modifications and discards the remaining ones.",
      "The database rolls back all changes made by the transaction, ensuring that no partial state is ever persisted.",
      "The database pauses operations until an administrator manually restarts the transaction."
    ],
    answer: "The database rolls back all changes made by the transaction, ensuring that no partial state is ever persisted.",
    explanation: "Atomicity is 'all-or-nothing'. If a transaction is interrupted or fails at any point, all intermediate modifications are completely undone."
  },
  {
    question: "How does a database engine resolve a Deadlock between concurrent transactions?",
    options: [
      "It pauses both transactions indefinitely until one naturally releases its lock.",
      "It monitors locks with a dependency graph, detects cycles, selects a 'victim' transaction to abort/rollback, and allows the other to proceed.",
      "It merges the two transactions into a single execution thread."
    ],
    answer: "It monitors locks with a dependency graph, detects cycles, selects a 'victim' transaction to abort/rollback, and allows the other to proceed.",
    explanation: "Since both transactions are blocked waiting on each other, the database must actively intervene. It detects the circular wait, aborts one transaction (releasing its locks), and reports the error to the client."
  },
  {
    question: "What is the primary architectural use case for the 'FOR UPDATE SKIP LOCKED' clause in SQL query locking?",
    options: [
      "To bypass index scans and execute queries faster.",
      "To implement lock-free high-frequency statistical aggregations.",
      "To build high-throughput job queues where multiple concurrent workers can safely dequeue distinct tasks without lock blocking."
    ],
    answer: "To build high-throughput job queues where multiple concurrent workers can safely dequeue distinct tasks without lock blocking.",
    explanation: "Normally, 'FOR UPDATE' blocks other readers. Adding 'SKIP LOCKED' instructs concurrent workers to skip over already-locked rows and fetch the next available pending job, avoiding contention and wait bottlenecks."
  }
]} />
