...
3. **RNN**: Build a simple LSTM model in PyTorch to classify the sentiment of movie reviews (binary classification) or predict the next number in a sequence. Use an embedding layer and experiment with hidden sizes.
4. **Hyperparameter tuning**: Starting from a baseline MNIST model, experiment with: learning rate (1e-2, 1e-3, 1e-4), batch size (32, 256), dropout rates (0, 0.3, 0.5). Plot training curves and explain the results.
